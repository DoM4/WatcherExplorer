package com.domenicoaumenta.watcherexplorer.models
 ...
 */
class RepositoriesResponseTest {

    @Test
    fun repositoriesResponseParsing() {
        val result = Gson().fromJson<RepositoriesResponse>(REPOSITORY_RESPONSE, RepositoriesResponse::class.java)
        Assert.assertNotNull(result)
        Assert.assertEquals(3, result.total_count)
        Assert.assertEquals(false, result.incomplete_results)

        val repoOwner = result.items[0].owner
        Assert.assertNotNull(repoOwner)
        Assert.assertEquals("https://avatars0.githubusercontent.com/u/4783781?v=4", repoOwner.avatar_url)
        Assert.assertEquals("JetBrains", repoOwner.login)
    }
}
